(function () {
    ...
    angular.module('gn_map_directive', ['gn_owscontext_service'])
        ...
        .directive('gnDrawBbox', ['gnMap', 'gnOwsContextService', function (gnMap, gnOwsContextService) {
            return {
                restrict: 'A',
                replace: true,
                templateUrl: '../../catalog/components/common/map/' + 'partials/drawbbox.html',
                scope: {
                    htopRef: '@', hbottomRef: '@', hleftRef: '@', hrightRef: '@', dcRef: '@', lang: '=', location: '@'
                },
                link: function (scope, element, attrs) {
                    ...
                    if (attrs.hleft !== '' && attrs.hbottom !== '' && attrs.hright !== '' && attrs.htop !== '') {
                        scope.extent.md = [parseFloat(attrs.hleft), parseFloat(attrs.hbottom), parseFloat(attrs.hright), parseFloat(attrs.htop)];
                    }

                    var reprojExtent = function (from, to) {
                        scope.extent[to] = gnMap.reprojExtent(scope.extent[from], scope.projs[from], scope.projs[to]);
                    };
                    ...
                    scope.$watch('projs.form', function (newValue, oldValue) {
                        scope.extent.form = gnMap.reprojExtent(scope.extent.form, oldValue, newValue);
                    });
                    ...
                    var bboxLayer = new ol.layer.Vector({
                        source: source, style: boxStyle
                    });

                    var map = new ol.Map({
                        layers: [gnMap.getLayersFromConfig(), bboxLayer], renderer: 'canvas', view: new ol.View({
                            center: [0, 0], projection: scope.projs.map, zoom: 2
                        })
                    });

                    //Uses configuration from database
                    if (gnMap.getMapConfig().context) {
                        gnOwsContextService.loadContextFromUrl(gnMap.getMapConfig().context, map, true);
                    }

                    var dragbox = new ol.interaction.DragBox({
                        style: boxStyle, condition: function () {
                    ...
                    scope.updateBbox = function () {
                      ...
                      if (scope.extent.form[1] == -90) {
                        scope.extent.form[1] = -89;
                      }
                      if (scope.extent.form[3] == 90) {
                        scope.extent.form[3] = 89;
                      }
                    ...
                    scope.onRegionSelect = function (region) {
                        scope.$apply(function () {
                            scope.extent.md = [parseFloat(region.west), parseFloat(region.south), parseFloat(region.east), parseFloat(region.north)];
                            scope.location = region.name;
                          var oldSouth = scope.extent.md[1];
                          var oldNorth = scope.extent.md[3];
                          if (scope.extent.md[1] == -90) {
                            scope.extent.md[1] = -89;
                          }
                          if (scope.extent.md[3] == 90) {
                            scope.extent.md[3] = 89;
                          }
                          reprojExtent('md', 'map');
                            reprojExtent('md', 'form');
                            setDcOutput();
                            drawBbox();
                            map.getView().fitExtent(scope.extent.map, map.getSize());
                          scope.extent.md[1] = oldSouth;
                          scope.extent.md[3] = oldNorth;
