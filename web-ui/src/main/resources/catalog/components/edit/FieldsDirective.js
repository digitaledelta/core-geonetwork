(function() {
  ...
  /**
   * @ngdoc directive
   * @name gn_fields.directive:gnFieldHelpTooltip
   * @function
   *
   * @description
   * Initialized field label or fieldset legend tooltip
   ...
   * the tooltip is on focus, if not, the event is on click.
   *
   * @param {string} gnFieldHelpTooltip The tooltip configuration
   *  which identified the schema, the element name and optionally the
   *  element parent name and XPath. eg. 'iso19139|gmd:fileIdentifier'.
   *
   * @param {string} placement Tooltip placement. Default to 'bottom'
   ...
   */
  module.directive('gnFieldHelpTooltip', ['$translate', function ($translate) {
  return {
          restrict: 'A', link: function (scope, element, attrs) {
            var isInitialized = false;
            var isField = element.is('input') || element.is('textarea');
            element.on('$destroy', function () {
              element.off();
            });
              ...
              if (!isInitialized)
                ...
                // Retrieve field information (there is a cache)
                      ...
                      //html = attrs.gnFieldHelpTooltip;
                        ...
                        // to the remaining space between the element
                        // and the right window border.
                        var width = ($(window).width() - element.offset().left - element.outerWidth()) * .95;

                        var closeBtn = '<button onclick="$(this).' + 'closest(\'div.popover\').remove();" type="button" ' + 'class="fa fa-times btn btn-link pull-right"></button>';
                        ...
                        element.popover({
                          ...
                          html: true,
                          placement: placement,
                          template: '<div class="popover gn-popover" ' + 'style="max-width:' + width + 'px;' + 'width:' + width + 'px"' + '>' + '<div class="arrow">' + '</div><div class="popover-inner">' + closeBtn + '<h3 class="popover-title"></h3>' + '<div class="popover-content"><p></p></div></div></div>', //                       trigger: 'click',
                          trigger: isField ? 'focus' : 'click'
                        });
