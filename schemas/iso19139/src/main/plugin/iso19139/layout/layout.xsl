...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- Configure if a user is allowed to delete elements from the editor -->
    <xsl:variable name="isDeleteDisabled" select="true()"/>
    ...
    <xsl:template mode="mode-iso19139" priority="200"
      ...
      then gmd:PT_FreeText
        ...
        <xsl:variable name="labelElementName">
            <xsl:choose>
                <xsl:when test="$viewConfig/@name = 'default'"> <!-- only for the default view we use xpath labels -->
        ...
        <xsl:variable name="labelConfig"
                      select="gn-fn-metadata:getLabel($schema, $labelElementName, $labels, name(..), $isoType, $xpath)"/>
